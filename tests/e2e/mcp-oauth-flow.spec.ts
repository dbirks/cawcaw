...
test.describe('MCP OAuth Flow', () => {
  test('complete OAuth flow for HuggingFace MCP server', async ({ page }) => {
    ...
    if (!username || !password) {
      console.log('⚠️  TEST_HF_USERNAME and TEST_HF_PASSWORD environment variables not provided');
  ...
  test('handle OAuth flow errors gracefully', async ({ page }) => {
    ...
    await page
      ...
      .fill('https://invalid-oauth.example.com/mcp');
    ...
    await expect(
      page.getByText(
        /Network error|Connection Failed|Connection Error|Unable to reach server|error/i
      )
