...
test.describe('MCP OAuth Flow - Hugging Face', () => {
  ...
  test('should load app and navigate to settings', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="chat-view"], .chat-container, main')).toBeVisible({ timeout: TEST_TIMEOUT });
    
    // Look for settings button/link
    const settingsButton = page.locator('button, a').filter({ hasText: /settings|Settings|⚙/i }).first();
  ...
  test('should navigate to MCP settings tab', async ({ page }) => {
    ...
    await page.locator('button, a').filter({ hasText: /settings|Settings|⚙/i }).first().click();
    
    // Look for Tools & MCP tab
    const mcpTab = page.locator('button, [role="tab"]').filter({ hasText: /Tools.*MCP|MCP.*Tools|MCP/i }).first();
  ...
  test('should show quick setup options for Hugging Face MCP', async ({ page }) => {
    ...
    await page.locator('button, a').filter({ hasText: /settings|Settings|⚙/i }).first().click();
    await page.locator('button, [role="tab"]').filter({ hasText: /Tools.*MCP|MCP.*Tools|MCP/i }).first().click();
  ...
  test('should initiate Hugging Face MCP OAuth flow', async ({ page }) => {
    ...
    const connectButton = page.locator('button').filter({ hasText: /Connect|Setup|Add.*Hugging Face|Hugging Face.*Connect/i }).first();
    ...
    try {
      ...
      const usernameField = page.locator('input[name="username"], input[type="email"], input[placeholder*="username"], input[placeholder*="email"]').first();
      ...
      if (await usernameField.isVisible()) {
        ...
        // Submit form
        const submitButton = page.locator('button[type="submit"], input[type="submit"], button').filter({ hasText: /sign in|login|authorize/i }).first();
        await submitButton.click();
      }
      
      // Wait for redirect back to app
      await page.waitForURL(new RegExp(TEST_BASE_URL.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')), { timeout: TEST_TIMEOUT });
      ...
      if (popup) {
        ...
        const popupUsernameField = popup.locator('input[name="username"], input[type="email"], input[placeholder*="username"], input[placeholder*="email"]').first();
        const popupPasswordField = popup.locator('input[name="password"], input[type="password"], input[placeholder*="password"]').first();
        ...
        if (await popupUsernameField.isVisible()) {
          ...
          const popupSubmitButton = popup.locator('button[type="submit"], input[type="submit"], button').filter({ hasText: /sign in|login|authorize/i }).first();
    ...
    await expect(page.locator('text=/connected|success|✓|✅/i, [data-status="connected"], .status-connected')).toBeVisible({ timeout: TEST_TIMEOUT });
  });
  ...
  test('should handle OAuth errors gracefully', async ({ page }) => {
    ...
    if (await connectButton.isVisible()) {
      ...
      const errorElements = page.locator('text=/error|failed|invalid|unauthorized/i, [data-status="error"], .status-error, .error-message');
  ...
  test('should display MCP server status correctly', async ({ page }) => {
    ...
    const statusElements = page.locator('[data-status], .status, text=/status|connected|disconnected|connecting/i');
    ...
    const statusClasses = await statusElements.first().getAttribute('class') || '';
