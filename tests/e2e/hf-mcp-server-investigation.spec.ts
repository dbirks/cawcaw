...
test.describe('HuggingFace MCP Server Investigation - 406 Mystery Solved', () => {
  ...
  test('comprehensive HF MCP server investigation - 406 error analysis', async ({ page }) => {
    ...
    if (await existingServer.isVisible()) {
      ...
      await page
        ...
        .fill('Investigation server to solve 406 mystery');

      // Ensure HTTP Streamable transport is selected (should be default)
      const transportSelect = page.locator(
        'select:has-text("Transport"), combobox:has-text("Transport")'
      );
      ...
      page.on('console', (msg) => {
        if (
          msg.type() === 'error' ||
          msg.text().includes('406') ||
          msg.text().includes('MCPTest')
        ) {
      ...
      await expect(
        page.getByText('Connection Successful').or(page.getByText('Connection Failed'))
      ...
      await expect(
        page
          .getByText('HuggingFace MCP Investigation Server')
          .or(page.getByText('Hugging Face MCP Server'))
      ).toBeVisible();
    ...
    if (await oauthButton.isVisible()) {
      ...
      const isOAuthFlow =
        url.includes('huggingface.co') || (url.includes('oauth') && url.includes('callback'));
      ...
      const containsHFOAuth =
        oauthTab.url().includes('huggingface.co') &&
      ...
      const hasStateParam =
        oauthTab.url().includes('state=') || oauthTab.url().includes('state%3D');
  ...
  test('verify MCP server error handling and UI responsiveness in mobile', async ({ page }) => {
    ...
    await page
      ...
      .fill('https://invalid-mcp-server.example.com');
    ...
    await expect(
      page.getByText('Connection Failed').or(page.getByText('Connection Error'))
    ).toBeVisible({ timeout: 10000 });
